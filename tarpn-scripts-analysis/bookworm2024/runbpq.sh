...
if [ $? -ge 1 ]; then
    sudo rm -rf /usr/tarpn/sbin/home_web_app/remove_me_to_stop_server.txt   ## stop a TARPN-HOME v2.02 or earlier
...
echo "runbpq.sh:0: called by " $(whoami)  >> $RUNBPQLOG
sudo chmod 666 $RUNBPQLOG
sudo chown pi $RUNBPQLOG
...
#### Version Bookworm005 -- 2025-01-26 rename the log file alias from SCRIPTLOGFILE to RUNBPQLOG    Add kill and call of tarpn-mon  through the tarpnmon-run.sh
#### Version Bookworm006 -- 2025-01-26  fix bug where tarpnmon-runner was called using a path of /usr/lo cal/sbin instead of /usr/tarpn/sbin
#### Version Bookworm007 -- 2025-05-09  delete start of tarpnmon-runner.  This start is now handled by tarpn-mon-service

#### VERSION NUMBER
echo -ne $(date) " " >> $RUNBPQLOG
echo "#### =RUNBPQ.SH                 Bookworm007 ="; #  --VERSION--#########
echo                     "###  RUNBPQ Bookworm007"      >> $RUNBPQLOG;
uptime  >> $RUNBPQLOG;
...
#### Get all of the USB/serial devices and find out their KAUP8Rs.  Update the /usr/tarpn/etc  kauper files.
...
then
   ...
   NODE_START_TIME="/usr/tarpn/etc/node_start_time.txt"
