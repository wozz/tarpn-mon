...
## bookworm010 1/29/2024   Change paths to /usr/tarpn/etc   Get rid of chat neighbor callsigns.
## bookworm011 1/30/2024   Change the order of the prompts so all of the callsigns come first.
## bookworm012 1/31/2024   Fix bug where writes to the logfile were to /home/pi/TARPN_COMMAND_LOGFILE instead of to /var/log/tarpn_command.log
## bookworm013 1/31/2024   Add switch to declare ttyUSB vs ttyACM as standard USB device
## bookworm014 1/13/2025   Fix a spelling error that Steve KM9G found.  Fix description of the group password.

echo -ne $(date) " " >> $TARPN_COMMAND_LOGFILE
echo "####"                                                                  #########
echo "####"                                                                  #########
echo "#### =CONFIGURE_NODE.SH         Bookworm014" #  --VERSION--            #########
echo "#### =CONFIGURE_NODE.SH         Bookworm014" >> $TARPN_COMMAND_LOGFILE #########
echo "####"                                                                  #########
echo "####"                                                                  #########
...
then
   ...
   ReadFigureFromNodeIniFile "ttyUSB_vs_ttyACM";    if [ $? -eq 0 ]; then ttyUSB_vs_ttyACM=$value; fi
...
echo "You should probably set this to the group password.  Once in a"
echo "while some other station in your network may need to do some quick action"
echo "on your node, like sendnodes, or blocking transmissions so having a common"
echo "password is a good thing.  Try to work as a group."
...
echo
echo "ERATA note: The prompt here will only show part of the lat-lon."
echo "            Enter the two values with , and space and ignore the"
echo "            default value, if you need to change this figure."
echo "###"
echo
...
do
   ...
   echo "--------------------------------------------------------------------------------"
   echo "--------------------------------------------------------------------------------"
   echo "The next part enables and configures the node neighbors which will connect to"
   echo "your node using your standard NinoTNC.  The standard NinoTNC can be EITHER"
   echo "selected as KIT TNCs built using the Microchip MCP2221 USB interface, A3 or A4,"
   echo "which identifies itself to Linux as ttyACM,"
   echo "or it can be can be one of the TNCs, SMT, A1 or A2, which use the FTDI interface"
   echo "and identifies itself to Linux as ttyUSB."
   echo
   echo "There is a separate place later in the process to describe alternate interfaces,"
   echo "including other than your selected selected USB device-name.  There can be up to"
   echo "two alternate TNC ports.  Those are assigned last.  For most of your neighbors,"
   echo "up to 8 of them, you must choose that you'll be basing your node on the ttyUSB"
   echo "devices or ttyACM devices."
   echo
   echo "Enter 'ttyUSB' or 'ttyACM'  at this next prompt."
   newValueFor "Your standard TNC's device name" $ttyUSB_vs_ttyACM;              ttyUSB_vs_ttyACM=$value;
   if [ $ttyUSB_vs_ttyACM == "ttyACM" ];
   then _success=1;
   fi
   if [ $ttyUSB_vs_ttyACM == "ttyUSB" ];
   then _success=1;
   fi
done
...
echo "The order of neighbors is unimportant.  The node-program will figure out"
...
echo "DO NOT USE these for NinoTNC."
...
echo "non-traditional TNCs but may be used for NinoTNCs."
echo
...
echo "ttyUSB_vs_ttyACM:"$ttyUSB_vs_ttyACM >> $NODEINI_INPROG
...
echo -ne $(date) " " >> $TARPN_COMMAND_LOGFILE
echo "CONFIGURE COMPLETED"  >> $TARPN_COMMAND_LOGFILE
