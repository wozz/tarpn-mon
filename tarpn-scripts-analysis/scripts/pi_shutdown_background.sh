...
#### 11-22-2025 Bullseye031  Respect the $STOP_SERVICE_SCRIPT_EXECUTION_SEMAPHORE
####

echo -e "\n\n\n\n"  >> $TARPN_CONTROL_PANEL_LOGFILE
echo -ne $(date) "" >> $TARPN_CONTROL_PANEL_LOGFILE
echo "--VERSION--pi_shutdown_background    Bullseye031 - start" >> $TARPN_CONTROL_PANEL_LOGFILE
...
do
   ...
   then
      echo -ne $(date) " " >> $TARPN_CONTROL_PANEL_LOGFILE
      echo "exit for STOP-SERVICE-SEMAPHORE"
      exit 0;
   fi
