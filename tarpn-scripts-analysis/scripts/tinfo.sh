...
### bullseye 004   Fix the no-TNCs-reporting file-not-found error.
### bullseye 005   Add date to the end of TINFO.  Add display of ChatConfig.cfg  get the install date, update date and update-apps dates to be the same format.
### bullseye 006   Leave the chatconfig.cfg output on one line
...
PORTCALLRELATEFILEPREFIX="/tmp/tarpn/portcallrelate"
   ...
   then
       echo $(ls -l /usr/local/etc/update_last_completed.txt | cut -b23-34)
   else
       echo "NEVER"
   fi
