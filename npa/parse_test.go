...
// The chat node needs a destination of its own or peers cannot route replies
// to it. LinBPQ will not accept a broadcast for this over NetROM-over-TCP, so
// npa registers it with the NODES ADD sysop command instead.
func TestChatIdentityPrefersConfiguredCall(t *testing.T) {
	dir := t.TempDir()
	chat := filepath.Join(dir, "tarpn-chat.toml")
	if err := os.WriteFile(chat, []byte("[node]\ncall = \"WA2M-9\"\nalias = \"ZA2M09\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	call, alias, err := chatIdentity(map[string]string{"CHAT_CALL": "WA2M-9"}, chat)
	if err != nil {
		t.Fatal(err)
	}
	if call != "WA2M-9" || alias != "ZA2M09" {
		t.Fatalf("got %s:%s, want WA2M-9:ZA2M09", alias, call)
	}
}

// With no CHAT_CALL the TARPN convention is the node's base call with SSID -9.
func TestChatIdentityDerivesFromNodeCall(t *testing.T) {
	dir := t.TempDir()
	chat := filepath.Join(dir, "tarpn-chat.toml")
	if err := os.WriteFile(chat, []byte("[node]\nalias = \"ZA2M09\"\n"), 0o644); err != nil {
		t.Fatal(err)
	...
	call, _, err := chatIdentity(map[string]string{"NODE_CALL": "WA2M-2"}, chat)
	if err != nil {
		t.Fatal(err)
	}
	if call != "WA2M-9" {
		t.Fatalf("call = %q, want WA2M-9", call)
	}
}

// The alias is what tarpn-chat announces itself as, so it comes from that file.
// Without it NODES ADD cannot be formed at all - the command takes ALIAS:CALL.
func TestChatIdentityNeedsAlias(t *testing.T) {
	dir := t.TempDir()
	chat := filepath.Join(dir, "tarpn-chat.toml")
	if err := os.WriteFile(chat, []byte("[node]\ncall = \"WA2M-9\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if _, _, err := chatIdentity(map[string]string{"CHAT_CALL": "WA2M-9"}, chat); err == nil {
		t.Fatal("expected an error when the alias is missing")
	}
}

// readConf has to cope with tarpn-chat.toml as well as node.conf: section
// headers skipped, quotes stripped by the caller, ';' comments ignored.
func TestReadConfHandlesTomlSections(t *testing.T) {
	dir := t.TempDir()
	...
	if _, ok := got["[node]"]; ok {
		t.Error("section header leaked into the map")
	}
}
